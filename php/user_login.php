...
<div class="container mt-4">
    <div class="text-end">
        ...
        <a href="landing_page.php" class="btn btn-primary">Log Out</a>
    </div>
    ...
    <div class="container mt-5">
        ...
        <form method="POST" class="mb-4">
            <div class="mb-3">
                ...
                <input type="text" name="title" class="form-control bg-dark text-light" placeholder="List Title" required>
            </div>
            <div class="mb-3">
                <label class="text-light">Description</label>
                <textarea name="description" class="form-control bg-dark text-light" placeholder="Description"></textarea>
            </div>
            <button type="submit" name="create_list" class="btn btn-success"><i class="fas fa-plus"></i> Create List</button>
        </form>
            ...
            <?php while ($list = $lists->fetch_assoc()): ?>
                <div class="card bg-secondary text-light mb-3 list-card">
                    <div class="card-body">
                        ...
                        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#listModal<?= $list['id'] ?>">View</button>
                        <a href="?delete_list=<?= $list['id'] ?>" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this list?');">Delete</a>
                    </div>
                </div>
                
                <div class="modal fade" id="listModal<?= $list['id'] ?>" data-bs-backdrop="static" tabindex="-1" aria-labelledby="listModalLabel<?= $list['id'] ?>" aria-hidden="true">
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content bg-dark text-light">
                            <div class="modal-header">
                                <h5 class="modal-title" id="listModalLabel<?= $list['id'] ?>"><?= htmlspecialchars($list['title']) ?></h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <p>📝 <?= htmlspecialchars($list['description']) ?></p>
                                
                                <button class="btn btn-secondary mb-3" type="button" data-bs-toggle="collapse" data-bs-target="#editForm<?= $list['id'] ?>" aria-expanded="false" aria-controls="editForm<?= $list['id'] ?>">
                                    Edit List
                                </button>
                                <div class="collapse mb-3" id="editForm<?= $list['id'] ?>">
                                    <form method="POST">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="new_title" class="form-control" value="<?= htmlspecialchars($list['title']) ?>">
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label">Description</label>
                                            <textarea name="new_description" class="form-control"><?= htmlspecialchars($list['description']) ?></textarea>
                                        </div>
                                        <button type="submit" name="update_list" class="btn btn-primary">Save Changes</button>
                                    </form>
                                </div>
                                    ...
                                    <div class="row">
                                        <?php foreach ($movies as $movie): ?>
                                            <div class="col-4 text-center">
                                                <a href="movies.php?id=<?= $movie['movieId'] ?>" class="text-decoration-none">
                                                    <img src="<?= $movie['posterUrl'] ?>" alt="<?= htmlspecialchars($movie['title']) ?>" class="img-fluid rounded">
                                                    <p class="text-light small"><?= htmlspecialchars($movie['title']) ?></p>
                                                </a>
                                                <form method="POST" class="mt-1">
                                                    <input type="hidden" name="list_id" value="<?= $list['id'] ?>">
                                                    <input type="hidden" name="movie_id" value="<?= $movie['movieId'] ?>">
                                                    <button type="submit" name="delete_movie" class="btn btn-sm btn-danger"> <i class="bi bi-x"></i></button>
                                                </form>
                                            </div>
                                        <?php endforeach; ?>
                                    </div>
