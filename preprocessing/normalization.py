...
# Normalize genres (with ID first)
if "genres" in movies.columns:
    ...
    # Merge genreId into genres_movies
    genres_movies = genres_movies.merge(unique_genres, on="genres", how="left")
    genres_movies = genres_movies[['movieId', 'genreId']]
...
# Normalize languages (with ID first)
if "language" in movies.columns:
    ...
    # Merge languageId into languages_movies
    languages_movies = languages_movies.merge(unique_languages, on="language", how="left")
    languages_movies = languages_movies[['movieId', 'languageId']]
